...
lsi_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
	...
	/* Write the command. */
	writel(CONTROL_READ | CONTROL_PHYID(mii_id) | CONTROL_PHYREG(regnum),
	       base + MDIO_CONTROL);
...
lsi_mdio_write(struct mii_bus *bus, int mii_id, int regnum, u16 value)
{
	...
	/* Write the command. */
	writel(CONTROL_WRITE | CONTROL_PHYID(mii_id) |
	       CONTROL_PHYREG(regnum) | CONTROL_DATA(value),
	       base + MDIO_CONTROL);

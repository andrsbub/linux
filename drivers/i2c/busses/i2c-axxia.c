...
{
	u32 int_mask = readl(idev->base + MST_INT_ENABLE);
	int_mask &= ~mask;
	writel(int_mask, idev->base + MST_INT_ENABLE);
}
...
{
	struct i2c_msg *msg = idev->msg;
	size_t tx_fifo_avail = TX_FIFO_SIZE - readl(idev->base + MST_TX_FIFO);
...
axxia_i2c_isr(int irq, void *_dev)
{
	...
	if (status & MST_STATUS_SCC) {
		...
		if (status & MST_STATUS_AL)
			...
			idev->msg_err = -EIO;
		dev_dbg(idev->dev, "error %s, addr=%#x rx=%u/%u tx=%u/%u\n",
			status_str(status),
			idev->msg->addr,

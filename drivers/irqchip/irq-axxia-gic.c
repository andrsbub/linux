/*
 * drivers/irqchip/irq-axxia-gic.c
 *
 * Based on drivers/irqchip/irq-gic.c
 *
...
/* AXM IPI numbers. Offset by 32 (16 PPI + 16 SGI) */
enum axxia_ext_ipi_num {
	AXM_IPI_FIRST = 195+32,
	IPI0_CPU0 = AXM_IPI_FIRST,	/* Axm IPI 195 */
	IPI0_CPU1,
	IPI0_CPU2,
	IPI0_CPU3,
	...
	IPI3_CPU2,
	IPI3_CPU3,
	AXM_IPI_LAST = IPI3_CPU3
};

#define is_valid_ipi(_irqid) \
	(AXM_IPI_FIRST <= (_irqid) && (_irqid) <= AXM_IPI_LAST)
...
static struct gic_rpc_data gic_rpc_data;
...
static inline bool is_same_cluster(int cpu1, int cpu2)
{
	int pcpu1 = cpu_logical_map(cpu1);
	int pcpu2 = cpu_logical_map(cpu2);
	return MPIDR_AFFINITY_LEVEL(pcpu1, 1) == MPIDR_AFFINITY_LEVEL(pcpu2, 1);
}

static inline bool is_this_cluster(int cpu)
{
	return is_same_cluster(cpu, smp_processor_id());
}

static void muxed_ipi_message_pass(const struct cpumask *mask,
				   unsigned int ipi_num)
{
	...
	for_each_cpu(cpu, mask) {
		info = &per_cpu(ipi_mux_msg, cpu);
		set_bit(ipi_num, &info->msg);
	}
}

static void axxia_demux_ipi(unsigned int irqnr, struct irq_desc *desc)
{
	...
	writel_relaxed(irqnr, gic_data_cpu_base(gic) + GIC_CPU_EOI);
...
static void gic_mask_unmask(struct irq_data *d, bool do_mask)
{
	 ...
	 */
	if (is_this_cluster(irq_cpuid[irqid])) {
		_gic_mask_irq(d, do_mask);
	} else {
		if (do_mask)
...
static int gic_set_type(struct irq_data *d, unsigned int type)
{
	...
	gic_rpc_data.cpu = smp_processor_id();
			     ...
			     bool do_clear)
{
	 ...
	 */
	bit = 1 << (MPIDR_AFFINITY_LEVEL(cpu_logical_map(cpu), 0) + shift);
	mask = 0xff << shift;
			    ...
			    bool force)
{
	 ...
	 */
	if (irqid != IRQ_PMU &&
	    !is_same_cluster(cpu, irq_cpuid[irqid])) {
		/*
		 * If old cpu assignment falls within another cluster as
		 * the cpu we're currently running on, use RPC mechanism.
		 */
		if (!is_this_cluster(irq_cpuid[irqid])) {
...
static u32 get_cluster_id(void)
{
	...
	return MPIDR_AFFINITY_LEVEL(pcpu, 1);
}
...
static void __init gic_axxia_init(struct gic_chip_data *gic)
{
	 ...
	 * interrupts are initially assigned to cpu 0.
	 */
...
static void __cpuinit gic_dist_init(struct gic_chip_data *gic)
{
	 ...
	 */
	for (i = AXM_IPI_FIRST; i <= AXM_IPI_LAST; i++) {
		u8 cpumask_8 = 1 << ((i - IPI0_CPU0) % 4);
...
void axxia_gic_raise_softirq(const struct cpumask *mask, unsigned int irq)
{
	 ...
	 * arch/arm/kernel/smp.c) to an Axxia IPI interrupt.  The Axxia sends
	 * IPI interrupts to other cores via the use of "IPI send" registers.
	 * Each register is specific to a sending CPU and IPI number.
	 *
	 * For example:
	 * regoffset 0x0000 = CPU0 uses to send IPI0 to other CPUs
	 * regoffset 0x0004 = CPU0 uses to send IPI1 to other CPUs
	 * ...
	 * regoffset 0x1000 = CPU1 uses to send IPI0 to other CPUs
	 * regoffset 0x1004 = CPU1 uses to send IPI1 to other CPUs
	 * ...
	 */
	regoffset = this_cpu * 0x1000 + (this_cpu < 8 ? 0 : 0x8000);

	muxed_ipi_message_pass(mask, irq);
...
static int axxia_gic_secondary_init(struct notifier_block *nb,
				    unsigned long action, void *data)
{
	...
	if (action == CPU_STARTING || action == CPU_STARTING_FROZEN) {
		...
		/* Primary core in each cluster initialize the dist interface
		 */
		if (MPIDR_AFFINITY_LEVEL((pcpu), 0) == 0)
			gic_dist_init(gic);
...
static struct notifier_block gic_cpu_notifier = {
	.notifier_call = axxia_gic_secondary_init,
	.priority = 100,
};

static void __init axxia_gic_init_bases(int irq_start,
					void __iomem *dist_base,
					void __iomem *cpu_base,
					struct device_node *node)
{
	...
	for (irq = AXM_IPI_FIRST; irq <= AXM_IPI_LAST; ++irq)
		irq_set_handler(irq, axxia_demux_ipi);
...
static int __init axxia_gic_of_init(struct device_node *node,
...
IRQCHIP_DECLARE(axxia_gic, "lsi,axxia-gic", axxia_gic_of_init);
